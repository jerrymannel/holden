...
e.insertDocument = async (_collection, _data) => {
	...
	return await db.collection(_collection).insertOne(_data)
}

e.insertDocuments = async (_collection, _data) => {
	logger.trace(`insertDocuments :: ${_collection} :: ${JSON.stringify(_data)}`)
	return await db.collection(_collection).insertMany(_data)
}

e.updateDocument = async (_collection, _id, _data) => {
	logger.trace(`updateDocument :: ${_collection} :: ${_id}`)
	logger.trace(`updateDocument :: ${_collection} :: ${JSON.stringify(_data)}`)
	return await db.collection(_collection).updateOne({ '_id': _id }, { "$set": _data })
}

e.upsertDocument = async (_collection, _id, _data) => {
	logger.trace(`upsertDocument :: ${_collection} :: ${JSON.stringify(_id)}`)
	logger.trace(`upsertDocument :: ${_collection} :: ${JSON.stringify(_data)}`)
	return await db.collection(_collection).updateOne({ '_id': _id }, { "$set": _data }, {upsert: true})
}

e.rawUpdateDocument = async (_collection, _id, _data) => {
	logger.trace(`updateDocument :: ${_collection} :: ${_id}`)
	logger.trace(`updateDocument :: ${_collection} :: ${JSON.stringify(_data)}`)
	return await db.collection(_collection).updateOne({ '_id': _id }, _data)
}

e.deleteDocument = async (_collection, _filter) => {
	logger.trace(`deleteDocument :: ${_collection} :: ${JSON.stringify(_filter)}`)
	return await db.collection(_collection).deleteMany(_filter)
}

e.aggregate = async (_collection, _aggregation) => {
	logger.trace(`aggregate :: ${_collection} :: ${JSON.stringify(_aggregation)}`)
	return await db.collection(_collection).aggregate(_aggregation)
}
